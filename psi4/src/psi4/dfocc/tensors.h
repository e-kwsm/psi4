...
class Tensor1d {
   ...
   public:
    ...
    void gemv(bool transa, int m, int n, const SharedTensor2d &a, const SharedTensor2d &b, int start_a, int start_b,
              double alpha, double beta);
    void gemv(bool transa, int m, int n, const SharedTensor2d &a, const SharedTensor2d &b, int start_a, int start_b,
              int start_c, double alpha, double beta);
...
class Tensor2d {
   ...
   public:
    ...
    void contract323(bool transa, bool transb, int m, int n, const SharedTensor2d &a, const SharedTensor2d &b,
                     double alpha, double beta);
    // contract233: C[Q](m,n) = \sum_{k} A(m,k) * B[Q](k,n)
    void contract233(bool transa, bool transb, int m, int n, const SharedTensor2d &a, const SharedTensor2d &b,
                     double alpha, double beta);
    // contract332: C(m,n) = \sum_{k} A[Q](m,k) * B[Q](k,n)
    void contract332(bool transa, bool transb, int k, const SharedTensor2d &a, const SharedTensor2d &b, double alpha,
                     double beta);
    // contract424: Z(pq,rs) = \sum_{o} X(pq,ro) * Y(o,s): where target_x = 4, target_y = 1, and Z = A2d_
    void contract424(int target_x, int target_y, const SharedTensor2d &a, const SharedTensor2d &b, double alpha,
                     double beta);
    // contract442: C(p,q) \sum_{rst} A(pr,st) B(qr,st) , where row/col pair indices are related to B and sorted B.
    void contract442(int target_a, int target_b, const SharedTensor2d &a, const SharedTensor2d &b, double alpha,
                     double beta);
    ...
    void reg_denom_os(int frzc, int occA, int occB, const SharedTensor2d &fockA, const SharedTensor2d &fockB,
                      double reg);
    ...
    void cont444(int t_a1, int t_a2, int f_a1, int f_a2, const SharedTensor2d &A, int t_b1, int t_b2, int f_b1,
                 int f_b2, const SharedTensor2d &B, double alpha, double beta);
    void cont444(bool delete_a, int t_a1, int t_a2, int f_a1, int f_a2, SharedTensor2d &A, bool delete_b, int t_b1,
                 int t_b2, int f_b1, int f_b2, SharedTensor2d &B, double alpha, double beta);
    ...
    void cont444(std::string idx_c, std::string idx_a, std::string idx_b, SharedTensor2d &A, SharedTensor2d &B,
                 double alpha, double beta);
    ...
    void cont424(std::string idx_c, std::string idx_a, std::string idx_b, bool delete_a, SharedTensor2d &A,
                 SharedTensor2d &B, double alpha, double beta);
    // C(pq,rs) = \sum_{t} A(p,t) B(tq,rs)
    void cont244(std::string idx_c, std::string idx_a, std::string idx_b, bool delete_b, SharedTensor2d &A,
                 SharedTensor2d &B, double alpha, double beta);
    // C(Q,pq) = \sum_{rs} A(Q,rs) B(rs,pq)
    // where dim(idx_c) & dim(idx_a)=2 but dim(idx_b)=4
    void cont343(std::string idx_c, std::string idx_a, std::string idx_b, bool delete_b, SharedTensor2d &A,
                 SharedTensor2d &B, double alpha, double beta);
    // C(Q,pq) = \sum_{r} A(p,r) B(Q,rq)
    void cont233(std::string idx_c, std::string idx_a, std::string idx_b, SharedTensor2d &A, SharedTensor2d &B,
                 double alpha, double beta);
    // C(Q,pq) = \sum_{r} A(Q,pr) B(r,q)
    void cont323(std::string idx_c, std::string idx_a, std::string idx_b, bool delete_a, SharedTensor2d &A,
                 SharedTensor2d &B, double alpha, double beta);
