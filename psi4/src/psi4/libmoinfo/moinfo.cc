...
MOInfo::MOInfo(Wavefunction& ref_wfn_, Options& options_, bool silent_ /* = false*/) : MOInfoBase(ref_wfn_, options_) {
  ...
  ***************/
    ...
    if (options_["CORR_MULTP"].has_changed()) multiplicity = options_.get_int("CORR_MULTP");
...
void MOInfo::read_mo_spaces() {
    ...
    if (nactv < nactive_ael) {
        ...
        error_msg +=
            " is smaller than the number of active alpha electrons (nactive_ael =" + to_string(nactive_ael) + ")",
            active_space_problem = true;
    }
    if (nactv < nactive_bel) {
        error_msg += "\n  - the number of active orbitals (nactv = " + to_string(nactv) + ")";
        error_msg +=
            " is smaller than the number of active beta electrons (nactive_bel =" + to_string(nactive_bel) + ")",
            active_space_problem = true;
    }
  ...
  ****************************************************************/
    ...
    // Set the mappings
    for (int i = 0; i < nall; ++i) mo_to_all[all_to_mo[i]] = i;
