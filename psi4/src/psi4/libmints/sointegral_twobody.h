...
class PSI_API TwoBodySOInt {
   ...
   public:
    ...
    TwoBodySOInt(const std::shared_ptr<TwoBodyAOInt> &aoint, const std::shared_ptr<IntegralFactory> &intfac,
                 const CdSalcList &cdsalcs);
    TwoBodySOInt(const std::vector<std::shared_ptr<TwoBodyAOInt> > &tb,
                 const std::shared_ptr<IntegralFactory> &integral, const CdSalcList &cdsalcs);
    ...
    void compute_shell(const SOShellCombinationsIterator &shellIter, TwoBodySOIntFunctor &body) {
        compute_shell(shellIter.p(), shellIter.q(), shellIter.r(), shellIter.s(), body);
    }
...
void TwoBodySOInt::compute_shell(int uish, int ujsh, int uksh, int ulsh, TwoBodySOIntFunctor &body) {
    ...
    dprintf("istab %d, jstab %d, kstab %d, lstab %d, ijstab %d, klstab %d\n", istabdense, jstabdense, kstabdense,
            lstabdense, ijstablizer, klstablizer);
    ...
    for (int ij = 1; ij <= R_size; ++ij) {
        ...
        for (int ijkl = 1; ijkl <= T_size; ++ijkl) {
            ...
            for (int kl = 1; kl <= S_size; ++kl) {
                ...
                // Check AM
                int total_am = tb_[thread]->basis1()->shell(si).am() + tb_[thread]->basis2()->shell(sj).am() +
                               tb_[thread]->basis3()->shell(sk).am() + tb_[thread]->basis4()->shell(sl).am();

                if (!(total_am % 2) || (siatom != sjatom) || (sjatom != skatom) || (skatom != slatom)) {
                    sj_arr.push_back(sj);
                    sk_arr.push_back(sk);
                    sl_arr.push_back(sl);
                }
            ...
            for (ktr = 0; ktr < n3; ktr++) {
                ...
                for (ltr = 0; ltr < n4; ltr++) {
                    ...
                    if (std::fabs(buffer_[thread][lsooff]) > cutoff_) {
                        ...
                        // func off/on
                        body(iiabs, jjabs, kkabs, llabs, iiirrep, iirel, jjirrep, jjrel, kkirrep, kkrel, llirrep, llrel,
...
void TwoBodySOInt::compute_shell_deriv1(int uish, int ujsh, int uksh, int ulsh, TwoBodySOIntFunctor &body) {
    ...
    for (size_t i = 0; i < cdsalcs_->ncd(); ++i) ::memset(deriv_[thread][i], 0, sizeof(double) * nso);
                        ...
                        for (int ksym = 0; ksym < nirrep; ++ksym) {
                            ...
                            for (int ktr = 0; ktr < nkfunc; ktr++) {
                                ...
                                for (int ltr = 0; ltr < nlfunc; ltr++) {
                                    ...
                                    if (only_totally_symmetric_ == true && total_symmetry != 0) continue;
                                    ...
                                    dprintf(
                                        "so' derivatives: A[0] %+lf A[1] %+lf A[2] %+lf\n"
                ...
                for (ltr = 0; ltr < n4; ltr++) {
                    ...
                    for (size_t i = 0; i < cdsalcs_->ncd(); ++i) {
                        if (std::fabs(deriv_[thread][i][lsooff]) > cutoff_)
                            body(i, iiabs, jjabs, kkabs, llabs, iiirrep, iirel, jjirrep, jjrel, kkirrep, kkrel, llirrep,
                                 llrel, deriv_[thread][i][lsooff]);
                    }
...
void TwoBodySOInt::compute_integrals(TwoBodySOIntFunctor &functor) {
    if (comm_ == "MADNESS") {
        throw PSIEXCEPTION(
            "PSI4 was not built with MADNESS. "
            "Please rebuild PSI4 with MADNESS, or "
            "change your COMMUNICATOR "
            "environment variable to MPI or LOCAL.\n");
    } else {
        std::shared_ptr<SOShellCombinationsIterator> shellIter =
            std::make_shared<SOShellCombinationsIterator>(b1_, b2_, b3_, b4_);
...
void TwoBodySOInt::compute_integrals_deriv1(TwoBodySOIntFunctor &functor) {
    if (!only_totally_symmetric_)
        throw PSIEXCEPTION(
            "The way the TPDM is stored and iterated enables only totally symmetric"
