...
AOTransform::AOTransform() {
    for (int h = 0; h < 8; h++) nfuncpi[h] = 0;
}

AOTransform::~AOTransform() {}

void AOTransform::add_transform(int irrep, double coef, int aofunc, int sofunc) {
...
void SOBasisSet::init() {
    ...
    for (i = 0; i < mol->nunique(); i++) {
        for (j = 0; j < basis_->nshell_on_center(mol->unique(i)); j++) {
            for (k = 0; k < mol->nequivalent(i); k++) {
                ...
                //                outfile->Printf( "i = %d j = %d k = %d aoshell = %d soshell = %d, mol->equivalent =
                //                %d\n",
    ...
    for (i = 0; i < nblocks; i++) {
      for (j = 0; j < soblocks[i].len(); j++) {
          if (soblocks[i].so[j].length == 0) continue;
            ...
            //            outfile->Printf( "i = %d j = %d bfn0 = %d aoshell0 = %d soshell0 = %d atom0 = %d nequiv0 =
    ...
    for (i = 0; i < nblocks; i++) {
        ...
        for (j = 0; j < soblocks[i].len(); j++) {
            ...
            int naofunc =
                include_pure_transform ? basis_->shell(aoshell0).ncartesian() : basis_->shell(aoshell0).nfunction();
            ...
            for (k = 0; k < soblocks[i].so[j].length; k++) {
                ...
                int aoshellfunc = bfn - (include_pure_transform ? basis_->shell_to_ao_function(aoshell)
        ...
        for (i = 0; i < nshell_; i++) {
            for (j = 0; j < nirrep_; j++) {
                for (k = 0; k < nfunc_[i][j]; k++, ibasis_++) {
                    ...
                    //                    ibasis_, func_within_irrep_[ibasis_]);
                }
            }
        }
    }
...
void SOBasisSet::print(std::string out) const {
    ...
    for (i = 0; i < nirrep_; i++) printer->Printf(" %3d", ncomp_[i]);
    printer->Printf(" ]\n");

    printer->Printf("    nfunc:\n");
    for (i = 0; i < nshell_; i++) {
        printer->Printf("      %3d:", i);
        for (j = 0; j < nirrep_; j++) printer->Printf("  %3d", nfunc_[i][j]);
        printer->Printf("\n");
    }
    ...
    for (i = 0; i < nshell_; i++) {
        printer->Printf("      %3d:", i);
        for (j = 0; j < nirrep_; j++) printer->Printf("  %3d", funcoff_[i][j]);
        printer->Printf("\n");
    }
        ...
        for (j = 0; j < sotrans_[i].naoshell; j++) {
          for (k = 0; k < sotrans_[i].aoshell[j].nfunc(); k++) {
                printer->Printf("      SO(%3d %2d %d [%2d]) += %12.8f * AO(%3d %2d)\n", i,
                                ...
                                sotrans_[i].aoshell[j].func[k].coef, sotrans_[i].aoshell[j].aoshell,
    ...
    for (i = 0; i < basis_->nshell(); ++i) {
        ...
        for (j = 0; j < (int)aotrans_[i].soshell.size(); ++j) {
            printer->Printf("      AO(%3d) sofunc %d aofunc %d irrep %d coef %12.8f\n", i,
                            aotrans_[i].soshell[j].sofunc, aotrans_[i].soshell[j].aofunc, aotrans_[i].soshell[j].irrep,
                            aotrans_[i].soshell[j].coef);
        }
    }
}

std::shared_ptr<BasisSet> SOBasisSet::basis() const { return basis_; }
