...
void CIWavefunction::get_parameters(Options &options) {
    ...
    if (Parameters_->dertype == "FIRST" || Parameters_->wfn == "DETCAS" || Parameters_->wfn == "CASSCF" ||
    ...
    if (options["S"].has_changed()) Parameters_->S = options.get_double("S");
    ...
    if (Parameters_->ref == "RHF") {
        ...
        throw InputException("Invalid DETCI reference " + Parameters_->ref, "REFERENCE", __FILE__, __LINE__);
    }

    if (options["MS0"].has_changed()) Parameters_->Ms0 = options["MS0"].to_integer();

    Parameters_->h0blocksize = options.get_int("H0_BLOCKSIZE");
    Parameters_->h0guess_size = Parameters_->h0blocksize;
    if (options["H0_GUESS_SIZE"].has_changed()) Parameters_->h0guess_size = options.get_int("H0_GUESS_SIZE");
    if (Parameters_->h0guess_size > Parameters_->h0blocksize) Parameters_->h0guess_size = Parameters_->h0blocksize;
    ...
    if (options["FCI"].has_changed()) Parameters_->fci = options["FCI"].to_integer();

    Parameters_->fci_strings = 0;
    if (Parameters_->fci) Parameters_->fci_strings = 1;
    if (options["FCI_STRINGS"].has_changed()) Parameters_->fci_strings = options["FCI_STRINGS"].to_integer();
    ...
    if (options["MPN"].has_changed()) Parameters_->mpn = options["MPN"].to_integer();
    ...
    Parameters_->perturbation_parameter = options.get_double("PERTURB_MAGNITUDE");

    if (Parameters_->perturbation_parameter <= 1.0 && Parameters_->perturbation_parameter >= -1.0)
        Parameters_->z_scale_H = 1;
    /*
    ...
    */
    if (options["MPN_SCHMIDT"].has_changed()) Parameters_->mpn_schmidt = options["MPN_SCHMIDT"].to_integer();

    if (options["MPN_WIGNER"].has_changed()) Parameters_->wigner = options["MPN_WIGNER"].to_integer();
    ...
    if (options["HD_OTF"].has_changed()) Parameters_->hd_otf = options["HD_OTF"].to_integer();

    if (Parameters_->hd_otf == 0) {
        outfile->Printf("Warning: HD_OTF FALSE has not been tested recently\n");
    }

    if (options["NO_DFILE"].has_changed()) Parameters_->nodfile = options["NO_DFILE"].to_integer();
    ...
    if (options["PRECONDITIONER"].has_changed()) {
        ...
        if (line1 == "GEN_DAVIDSON") Parameters_->precon = PRECON_GEN_DAVIDSON;
        if (line1 == "H0BLOCK") Parameters_->precon = PRECON_GEN_DAVIDSON;
        if (line1 == "H0BLOCK_INV") Parameters_->precon = PRECON_H0BLOCK_INVERT;
        if (line1 == "ITER_INV") Parameters_->precon = PRECON_H0BLOCK_ITER_INVERT;
        if (line1 == "H0BLOCK_COUPLING") Parameters_->precon = PRECON_H0BLOCK_COUPLING;
        if (line1 == "EVANGELISTI") Parameters_->precon = PRECON_EVANGELISTI;
    }
    ...
    if (Parameters_->diag_method < METHOD_DAVIDSON_LIU_SEM && Parameters_->update == UPDATE_DAVIDSON) {
        outfile->Printf(
            "DAVIDSON update not available for OLSEN or MITRUSH"
            " iterators\n");
        Parameters_->update = UPDATE_OLSEN;
    }
    if (Parameters_->precon == PRECON_EVANGELISTI &&
        (Parameters_->update != UPDATE_DAVIDSON || Parameters_->diag_method != METHOD_DAVIDSON_LIU_SEM)) {
        outfile->Printf(
            "EVANGELISTI preconditioner not available for OLSEN or"
    ...
    if (options["NUM_INIT_VECS"].has_changed()) Parameters_->num_init_vecs = options.get_int("NUM_INIT_VECS");
    ...
    } else if (Parameters_->maxnvect == 0 && Parameters_->diag_method == METHOD_OLSEN) {
    ...
    if (Parameters_->opdm_diag || Parameters_->opdm_ave) Parameters_->opdm = 1;
    if (options["OPDM"].has_changed()) Parameters_->opdm = options["OPDM"].to_integer();
    ...
    if (options["TDM"].has_changed()) Parameters_->transdens = options["TDM"].to_integer();

    /* dipole or transition dipole moment? */
    if (Parameters_->opdm)
        Parameters_->dipmom = 1;
    else
        Parameters_->dipmom = 0;

    if (Parameters_->transdens) Parameters_->dipmom = 1;

    if (Parameters_->wfn == "RASSCF" || Parameters_->wfn == "CASSCF" || Parameters_->wfn == "DETCAS")
        Parameters_->dipmom = 0;

    if (options["DIPMOM"].has_changed()) Parameters_->dipmom = options["DIPMOM"].to_integer();
    ...
    if (options["TPDM"].has_changed()) Parameters_->tpdm = options["TPDM"].to_integer();

    if (Parameters_->num_init_vecs < Parameters_->num_roots) Parameters_->num_init_vecs = Parameters_->num_roots;
    if (Parameters_->guess_vector == PARM_GUESS_VEC_UNIT && Parameters_->num_init_vecs > 1) {
        Parameters_->guess_vector = PARM_GUESS_VEC_H0_BLOCK;
        outfile->Printf(
            "Warning: Unit vec option not available for more than"
            " one root\n");
    }
    if (Parameters_->guess_vector == PARM_GUESS_VEC_UNIT) Parameters_->h0blocksize = Parameters_->h0guess_size = 1;
    ...
    if (options["FOLLOW_VECTOR"].has_changed()) {
        ...
        for (i = 0; i < Parameters_->follow_vec_num; i++) {
            ...
            Parameters_->follow_vec_Ib[i] = options["FOLLOW_VECTOR"][i][0][1].to_integer();
...
void CIWavefunction::print_parameters() {
    ...
    if (Parameters_->filter_guess || Parameters_->sf_restrict) {
        outfile->Printf("    FILTER GUESS   =   %6s      SF RESTRICT   =   %6s\n",
                        Parameters_->filter_guess ? "YES" : "NO", Parameters_->sf_restrict ? "YES" : "NO");
    }
    ...
    // LSE
    if (Parameters_->lse) {
        outfile->Printf("    LSE            =   %6s      LSE ITER      =   %6d\n", Parameters_->lse ? "YES" : "NO",
                        Parameters_->lse_iter);
    }
    ...
    for (i = 0; i < Parameters_->average_num; i++) {
        ...
        outfile->Printf("%2d(%4.2lf) ", Parameters_->average_states[i], Parameters_->average_weights[i]);
    }

    if (Parameters_->follow_vec_num > 0) {
        outfile->Printf("\nDensity matrices will follow vector like:\n");
        for (i = 0; i < Parameters_->follow_vec_num; i++)
            outfile->Printf("(%d %d) %12.6lf\n", Parameters_->follow_vec_Ia[i], Parameters_->follow_vec_Ib[i],
...
void CIWavefunction::set_ras_parameters() {
     ...
     */
    if (Parameters_->fci == 1 && (CalcInfo_->num_alp_expl + CalcInfo_->num_bet_expl) > Parameters_->ex_lvl) {
    ...
    Parameters_->a_ras1_max = (CalcInfo_->num_alp_expl > Parameters_->a_ras1_lvl + 1) ? Parameters_->a_ras1_lvl + 1
                                                                                      : (CalcInfo_->num_alp_expl);
    // CDS 4/15
    // if (Parameters_->fzc) Parameters_->a_ras1_max += CalcInfo_->num_fzc_orbs;

    Parameters_->b_ras1_max = (CalcInfo_->num_bet_expl > Parameters_->b_ras1_lvl + 1) ? Parameters_->b_ras1_lvl + 1
    ...
    if (Parameters_->a_ras3_max == -1 || Parameters_->b_ras3_max == -1) {
        if (Parameters_->ras3_max != -1) { /* have parsed ras3_max */
            Parameters_->a_ras3_max =
                (Parameters_->ras3_max <= CalcInfo_->num_alp_expl) ? Parameters_->ras3_max : CalcInfo_->num_alp_expl;
            Parameters_->b_ras3_max =
                (Parameters_->ras3_max <= CalcInfo_->num_bet_expl) ? Parameters_->ras3_max : CalcInfo_->num_bet_expl;
        } else {
            Parameters_->a_ras3_max =
                (Parameters_->ex_lvl <= CalcInfo_->num_alp_expl) ? Parameters_->ex_lvl : CalcInfo_->num_alp_expl;
            Parameters_->b_ras3_max =
                (Parameters_->ex_lvl <= CalcInfo_->num_bet_expl) ? Parameters_->ex_lvl : CalcInfo_->num_bet_expl;
        }
    }
    ...
    if (Parameters_->cc) {
        ...
        if (Parameters_->ras4_max != -1) { /* have parsed */
            Parameters_->a_ras4_max =
                (Parameters_->ras4_max <= CalcInfo_->num_alp_expl) ? Parameters_->ras4_max : CalcInfo_->num_alp_expl;
            Parameters_->b_ras4_max =
                (Parameters_->ras4_max <= CalcInfo_->num_bet_expl) ? Parameters_->ras4_max : CalcInfo_->num_bet_expl;
        } else {
    ...
    i = (CalcInfo_->num_alp_expl <= Parameters_->a_ras1_lvl + 1) ? CalcInfo_->num_alp_expl
                                                                 : Parameters_->a_ras1_lvl + 1;
    ...
    i = (CalcInfo_->num_bet_expl <= Parameters_->b_ras1_lvl + 1) ? CalcInfo_->num_bet_expl
                                                                 : Parameters_->b_ras1_lvl + 1;
    ...
    // }
    if (Parameters_->ras3_max == -1) {
        Parameters_->ras3_max = (Parameters_->ex_lvl <= tot_expl_el) ? Parameters_->ex_lvl : tot_expl_el;
    } else {
        if (Parameters_->ras3_max > tot_expl_el) Parameters_->ras3_max = tot_expl_el;
    }
    ...
    i = (tot_expl_el < 2 * (Parameters_->ras1_lvl + 1)) ? tot_expl_el : 2 * (Parameters_->ras1_lvl + 1);
    ...
    if (Parameters_->ras4_max == -1) {
        if (Parameters_->cc) {
            Parameters_->ras4_max =
                (Parameters_->cc_ras4_max + 2 <= tot_expl_el) ? Parameters_->cc_ras4_max + 2 : tot_expl_el;
        } else
            Parameters_->ras4_max = (Parameters_->ex_lvl <= tot_expl_el) ? Parameters_->ex_lvl : tot_expl_el;
    }
    ...
    if (Parameters_->a_ras34_max > CalcInfo_->num_alp_expl) Parameters_->a_ras34_max = CalcInfo_->num_alp_expl;
    if (Parameters_->b_ras34_max > CalcInfo_->num_bet_expl) Parameters_->b_ras34_max = CalcInfo_->num_bet_expl;
    // if (Parameters_->cc) {
